...
>
    ...
    <?php foreach ($paymentmethod as $pmethod) { ?>
        <option value="<?php echo html_escape($pmethod->payment_method_id); ?>">
            <?php echo html_escape($pmethod->payment_method); ?>
        </option>
    <?php } ?>
</select>
                    ...
                    <div class='row'>
                        <div class="col-12" id ="account_info"></div>
...
<script>
  // Inject CSRF token for use in external JS
  window.CSRF = {
...
<script src="<?php echo base_url('assets/js/process.js') ?>" type="text/javascript"></script>
