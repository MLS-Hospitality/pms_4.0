	...
	public function __construct()
{
    ...
    $this->load->model([
        ...
        'Roomreservation_model' => 'roomreservation_model'
    ]);

    $this->load->helper('captcha');

    $this->allmenu = $this->hotel_model->allmenu_dropdown();
    $this->webinfo = $this->db->get('common_setting')->row();
    $this->settinginfo = $this->db->get('setting')->row();
    $this->storecurrency = $this->db
        ->where('currencyid', $this->settinginfo->currency)
        ->get('currency')
        ->row();
}
	...
	public function checkout()
	{
		...
		$userinfo = $this->db->select("*")->from('customerinfo')->where('customerid', $this->session->userdata('UserID'))->get()->row();
		$data['userinfo'] = $userinfo;
		$data['paymentmethod'] = $this->db->select("*")->from('payment_method')->where('is_active', 1)->where('payment_method_id',1)->get()->result();
...
{
    /* ===================== BASIC DATA ===================== */
    $orderinfo = $this->hotel_model->read('*', 'booked_info', [
        'booking_number' => $orderid
    ]);

    $pmethod = (int) $paymentid;
    ...
    $binfo = $this->db->select("
        ...
        ->where("b.booking_number", $orderid)
        ->get()
        ->row();
    ...
    /* ===================== NIGHTS ===================== */
    $nights = (!empty($binfo->extra_facility_days) && $binfo->extra_facility_days > 0)
        ? (int) $binfo->extra_facility_days
        : 1;

    /* ===================== HOTEL SETTINGS ===================== */
    $hotel_settings = $this->db->get_where("common_setting", ["id" => 1])->row();
    $appNameRow     = $this->db->get_where("setting", ["id" => 2])->row();

    $hotelName          = $appNameRow->title ?? 'Hotel';
    $hotelEmail         = $hotel_settings->email ?? 'info@hotel.com';
    $hotelPhone         = $hotel_settings->phone ?? 'Contact Number';
    $hotelAddress       = $hotel_settings->address ?? 'Hotel Address';
    $hotelBankName      = $hotel_settings->bank_name ?? 'Bank';
    $hotelAccountNumber = $hotel_settings->account_number ?? '0000000000';
    $hotelAccountName   = $hotel_settings->account_name ?? $hotelName;

    /* ===================== PRICE ===================== */
    $room_rate = (float) ($binfo->roomrate ?? 0);
    $base_rent = $room_rate * $nights;

    $tax_amount = 0;
    if (method_exists($this->roomreservation_model, 'calculateTax')) {
        $tax = $this->roomreservation_model->calculateTax($base_rent, true);
        $tax_amount = $tax['total'] ?? 0;
    }

    $serviceChargeRate = $this->settinginfo->servicecharge??0;
$service_charge = 0;

if($serviceChargeRate>0){

	$service_charge = ($base_rent*$serviceChargeRate)/100;
}
    $discount_amount = (float) ($binfo->discountamount ?? 0);
    $grand_total = ($base_rent + $tax_amount + $service_charge) - $discount_amount;
  $greeting_message = "Thank you for choosing " . ($appName->title ?: 'our hotel') . ".";

    /* ===================== PAYMENT MESSAGE ===================== */
    if ($pmethod === 4) {
        // Bank Transfer
        $paymentMessage = "
            Kindly make payment of <strong>₦" . number_format($grand_total, 2) . "</strong> 
            to the account below:
            <br><br>
            <strong>Bank:</strong> {$hotelBankName}<br>
            <strong>Account Name:</strong> {$hotelAccountName}<br>
            <strong>Account Number:</strong> {$hotelAccountNumber}
        ";
    } elseif ($pmethod === 9) {
        // Cash
        $paymentMessage = "
            Kindly bring the total amount of 
            <strong>₦" . number_format($grand_total, 2) . "</strong> 
            to the reception upon arrival.
        ";
    } else {
        $greeting_message = "Thank you for choosing " . ($appName->title ?: 'our hotel') . ". We are pleased to confirm your booking with us in apartment " .$binfo->roomtype . ".";
        // Card
        $paymentMessage = "
    ...
    /* ===================== EMAIL ===================== */
    if (!empty($binfo->email)) {

        $guestName = ucwords(trim($binfo->firstname . ' ' . $binfo->lastname));
        $roomType  = $binfo->roomtype ?? 'Standard Room';
        $roomNo    = $binfo->room_no ?? 'N/A';
        $number_guest = $binfo->nuofpeople;

        $checkin  = $binfo->checkindate ? (new DateTime($binfo->checkindate))->format('jS F Y H:i') : 'N/A';
        $checkout = $binfo->checkoutdate ? (new DateTime($binfo->checkoutdate))->format('jS F Y H:i') : 'N/A';

        $subject = 'Booking Confirmation';
        $fmt_base_rent      = number_format($base_rent, 2);
$fmt_tax_amount     = number_format($tax_amount, 2);
$fmt_service_charge = number_format($service_charge, 2);
$fmt_discount       = number_format($discount_amount, 2);
$fmt_grand_total    = number_format($grand_total, 2);
...
<div class="card">
    ...
    <div class="section">
        <strong>Reservation:</strong> #{$binfo->booking_number}<br>
        <strong>Room:</strong> {$roomType} · {$roomNo}<br>
        <strong>Stay:</strong> {$checkin} – {$checkout}<br>
    ...
    <div class="section">
        ...
        <table class="table">
            <tbody>
                <tr>
                    <td>Guest</td>
                    <td class="amount">{$guestName}</td>
                </tr>
                <tr>
                    <td>Room</td>
                    <td class="amount">{$roomType} · {$roomNo}</td>
                </tr>
                <tr>
                    <td>Check-in</td>
                    <td class="amount">{$checkin}</td>
                </tr>
                <tr>
                    <td>Check-out</td>
                    <td class="amount">{$checkout}</td>
                </tr>
                <tr>
                    <td>Nights</td>
                    <td class="amount">{$nights}</td>
                </tr>
                <tr>
                    <td>Guests</td>
                    <td class="amount">{$number_guest}</td>
                </tr>
            </tbody>
        </table>
    </div>
    ...
    <div class="section">
        ...
        <table class="table">
            <tr>
                <td>Apartment Rate × {$nights}</td>
                <td class="amount">₦{$fmt_base_rent}</td>
            </tr>
            <tr>
                <td>Tax</td>
                <td class="amount">₦{$fmt_tax_amount}</td>
            </tr>
            <tr>
                <td>Service Charge</td>
                <td class="amount">₦{$fmt_service_charge}</td>
            </tr>
            <tr>
                <td>Discount</td>
                <td class="amount">- ₦{$fmt_discount}</td>
            </tr>
            <tr>
                <td><strong>Total</strong></td>
                <td class="amount"><strong>₦{$fmt_grand_total}</strong></td>
            </tr>
        </table>
    </div>
    ...
    <div class="section">
        <div class="note" style="background: #f0fcf7; border-color: rgba(31,111,84,0.2);">
            We are here 24/7. Reply to this email or call {$hotelPhone} for quick assistance.
        </div>
    </div>
...
HTML;
        ...
        $this->hotel_model->send_email(
            strtolower($binfo->email),
            $subject,
            $hotelName,
            $htmlContent,
            'booking'
...
/* ===================== WHATSAPP / REDIRECT ===================== */
$WhatsApp = $this->db
    ->where('directory', 'whatsapp')
    ->where('status', 1)
    ->get('module');

if ($WhatsApp->num_rows() === 1) {
    ...
    if ($this->db->table_exists('whatsapp_settings')) {
        ...
        if ($wtapp && (int) $wtapp->orderenable === 1) {

            if ($pmethod === 9) {
                echo json_encode(['status' => true, 'text' => 'whatsapp']);
                return;
            }
        }
    }
...
redirect('hotel/orderdelevered/' . $orderid);
	...
	public function orderdelevered($num = NULL)
	{
		...
		$data['title'] = "Booking Complete, Check your for inforamtion on next steps";
    ...
    // Make sure session userdata exists
    if ($this->session->userdata) {
        ...
        $payment_data['email'] = $user_data["UserEmail"];
        $payment_data['amount'] = $totalPrice;

        $payment_info = $this->generate_link($payment_data);
    ...
    // Optional: send error if session/cart is empty
    header('Content-Type: application/json');
    echo json_encode(['status' => 'error', 'message' => 'No cart or session data found']);
}
...
public function get_account_data(){
    ...
    if ($this->session->userdata) {
        ...
        foreach ($cart as $item) {
            $totalPrice += ceil((float) ($item['totalprice'] ?? 0));
        }

	
	
		$payment_data['bank_name'] = "Zenith Bank";
		$payment_data['bank_account_number'] = "1311267506"; 
		$payment_data['bank_account_name']= "The Aura serviced Apartment Ltd";
		$pyyment_data['status'] =true;

		// Get currency
		$currency = getCurrency();
		
}
$payment_data['bank_account_charge'] = trim("NGN") . ' ' . number_format((float)$totalPrice, 2);
 header('Content-Type: application/json');
