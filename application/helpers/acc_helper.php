...
function transaction($vno, $vType, $vDate, $COAID, $narration, $debit, $credit, $storeId, $isPosted, $createBy, $createDate, $isApprove, $bookedid = null, $invoice_number = null){
    ...
    if($insert & $isApprove==1){
        $head_type = $ci->db->select("HeadType,HeadCode")->from("acc_coa")->where("HeadCode", $COAID)->get()->row();
            ...
            if(!empty($ob_check)){
                ...
                // Liability/Income: Credit increases balance, Debit decreases
                ...
                $current_balance = floatval($ob_check->current_balance ?? 0);
                $debit_val = floatval($debit ?? 0);
                $credit_val = floatval($credit ?? 0);
                if ($head_type && ($head_type->HeadType == 'L' || $head_type->HeadType == 'I')) {
                    // Liability/Income: Credit increases balance, Debit decreases
                    $cr_balance = $current_balance - $debit_val + $credit_val;
                } else {
                    // Asset/Expense: Debit increases balance, Credit decreases
                    $cr_balance = $current_balance + $debit_val - $credit_val;
                }
                ...
                // Get account type for initial balance calculation
                $initial_balance = $debit - $credit;
                if ($head_type && ($head_type->HeadType == 'L' || $head_type->HeadType == 'I')) {
                    // Liability/Income: Reverse calculation
                    $initial_balance = $credit - $debit;
                }
