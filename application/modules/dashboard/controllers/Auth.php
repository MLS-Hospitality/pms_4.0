	...
	public function forgot_check()
	{
		...
		if ($checkemail) {
			...
			if(!$send_email || empty($send_email->smtp_host)){
				log_message('error', 'Email configuration not found or invalid');
				$notfound = array(
					'status'  => 'false',
					'msg'  => "Email configuration error. Please contact administrator.",
				);
			...
			// Suppress warnings for network errors and handle them gracefully
			...
			// Suppress the warning and check the result
			$result = @$this->email->send();
			
			if(!$result) {
				$error_msg = $this->email->print_debugger();
				log_message('error', 'Email sending failed. Error: ' . $error_msg);
				$notfound = array(
					'status'  => 'false',
					'msg'  => "Failed to send email. Please contact administrator.",
				);
				print_r(json_encode($notfound));
				return;
			}
