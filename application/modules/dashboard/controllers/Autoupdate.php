...
class Autoupdate extends MX_Controller {
 ...
 public function download_backup() {
        ...
        $backup_dir = FCPATH . 'assets/data/backup/';
        $save = $backup_dir . $db_name;

        // Create backup directory if it doesn't exist
        if (!is_dir($backup_dir)) {
            if (!mkdir($backup_dir, 0755, true)) {
                header('HTTP/1.0 500 Internal Server Error');
                echo json_encode(array('error' => 'Failed to create backup directory'));
                exit;
            }
            // Create index.php for security
            file_put_contents($backup_dir . 'index.php', '<?php defined(\'BASEPATH\') OR exit(\'No direct script access allowed\');');
        }

        // Check if directory is writable
        if (!is_writable($backup_dir)) {
            header('HTTP/1.0 500 Internal Server Error');
            echo json_encode(array('error' => 'Backup directory is not writable'));
            exit;
        }
        ...
        try {
            ...
            if ($b === FALSE) {
                header('HTTP/1.0 500 Internal Server Error');
                echo json_encode(array('error' => 'Failed to create database backup'));
                exit;
            }
            ...
            if (write_file($save, $backup) === FALSE) {
                header('HTTP/1.0 500 Internal Server Error');
                echo json_encode(array('error' => 'Failed to write backup file'));
                exit;
            }

            // Verify file was created
            if (!file_exists($save) || filesize($save) == 0) {
                header('HTTP/1.0 500 Internal Server Error');
                echo json_encode(array('error' => 'Backup file is empty or was not created'));
                exit;
            }

            $this->load->helper('download');
            force_download($save, NULL);

            // Clean up the file after download (optional - you may want to keep it)
            // @unlink($save);
