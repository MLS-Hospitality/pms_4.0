...
class Setting extends MX_Controller
{
    ...
    public function create_new()
    {
        ...
        $this->form_validation->set_rules('address', 'Address', 'max_length[255]|xss_clean');
        $this->form_validation->set_rules('email', 'Email', 'max_length[100]|valid_email|xss_clean');
        $this->form_validation->set_rules('phone', 'Phone', 'max_length[20]|xss_clean');
        $this->form_validation->set_rules('language', 'Language', 'max_length[250]|xss_clean');
        $this->form_validation->set_rules('footer_text', 'Footer Text', 'max_length[255]|xss_clean');
        ...
        if ($this->form_validation->run() === true) {
            ...
            if (empty($postData['id'])) {
                ...
                if ($test = $this->setting_model->update($postData)) {
                    #set success message
                    $this->session->set_flashdata('message', display('update_successfully'));
                    redirect('settings/2');
                } else {
    ...
    public function truncate_table()
    {
        ...
        // Get table name from POST - try both with and without XSS filtering
        $table = $this->input->post('table', true);
        if (empty($table)) {
            $table = $this->input->post('table', false);
        }

        // Also try raw input in case of issues
        if (empty($table) && isset($_POST['table'])) {
            $table = $_POST['table'];
        }
        ...
        try {
            ...
            if (!$this->db->table_exists($table)) {
                ...
                echo json_encode(['status' => 'error', 'message' => 'Table does not exist: ' . $table]);
                exit;
            }
            ...
            if ($this->db->truncate($table)) {
                ...
                echo json_encode(['status' => 'success', 'message' => 'Table ' . $table . ' truncated successfully']);
            } else {
                // Re-enable foreign key checks
                $this->db->query('SET FOREIGN_KEY_CHECKS = 1');
                header('HTTP/1.0 500 Internal Server Error');
                $error = $this->db->error();
                echo json_encode(['status' => 'error', 'message' => 'Failed to truncate table: ' . $table, 'db_error' => $error['message']]);
            }
        } catch (Exception $e) {
