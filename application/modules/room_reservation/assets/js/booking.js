...
var saveone = 1;
var num = 0;
var serverUrl = typeof baseurl !== "undefined"
    ? baseurl
...
$(document).ready(function() {
    ...
    function calculateBillingTotal(){
        ...
        // Calculate tax and service charge on base rent
        if(tax>0){
            tax = (tax*rent)/100;
        }
        if(scharge>0){
 ...
 $("#paymentmode").on('change', function(){
    ...
    // Consolidated into custom.js to avoid conflicts and support edit functionality
    /*
    $("#addcustomer,#addoldcustomer").on('click', function(){
        ...
        // Force Select2 to update its value
$('#nationality, #country').trigger('change.select2');
        if(exmobile==""){
    // Force Select2 to commit the selected value
    $('#nationality, #country').trigger('change.select2');

    var natVal = $('#nationality').select2('data')[0] 
        ? $('#nationality').select2('data')[0].id 
        : $('#nationality').val() || '';

    var countryVal = $('#country').select2('data')[0] 
        ? $('#country').select2('data')[0].id 
        : $('#country').val() || '';
    ...
    cols += '<td class="border-0" id="usernationality'+targetNum+'" hidden>'+natVal+'</td>';
    cols += '<td class="border-0" id="usercountry'+targetNum+'" hidden>'+countryVal+'</td>';  // ← added back
    ...
    cols += '<td class="border-0 pr-0 text-right"><button type="button" onclick="custedit('+targetNum+')" class="btn btn-info-soft btn-xs mr-1"><i class="fas fa-edit"></i></button><button type="button" onclick="custdel('+targetNum+')" id="custdel'+targetNum+'" class="btn btn-danger-soft btn-xs custdelete'+targetNum+'"><i class="far fa-trash-alt"></i></button></td>';
        ...
        if(ch==0){
            $("#pri"+targetNum).trigger("click");
        }

$("#firstname").val("");
$("#title").val($("#title option:first").val());
$("#code").val("");
$("#mobileNo").val("");
$("#email").val("");
$("#imgbbcak").val();
...
$("#lastname").val("");
$('input[name="customRadioInline"]:checked').prop("checked", false);
$("#country").val("");
$("#imgffront").val('');
$("#imggguest").val('');
...
$("#dob").val("");
$("#anniversary").val("");
$('#image-preview').attr('src', serverUrl + 'assets/img/proof_icon.png');

$("#nationality").val("");
$('input[name="vip"]:checked').prop("checked", false);
$("#pitype").val("");
$("#filename").text("Drag and Drop");

$("#pid").val("");
$('#image-preview2').attr('src', serverUrl + 'assets/img/proof_icon.png');
$("#filename2").text("Drag and Drop");
$("#comments").val("");
$("#filename3").text("Drag and Drop");

$('#image-preview3').attr('src', serverUrl + 'assets/img/user.png');
...
function getradio(nums){
    ...
    if(trl==null || trl==0){
        ...
        var pid = $("#userpid"+nums).text();var imgfront = $("#userimgfront"+nums).text();var imgback = $("#userimgback"+nums).text();
        var country = $("#usercountry"+nums).text()
