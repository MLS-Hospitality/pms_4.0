...
// Validation function to check if advance amount equals total amount
function validateAdvanceAmount() {
    ...
    var submitButton = $("#bookingsave");

    // Card payment MUST be full
    if (paymentmode === "Card Payment" && totalAmount > 0 && advanceAmount !== totalAmount) {
        submitButton.prop('disabled', true)
            .removeClass('btn-primary')
            .addClass('btn-secondary');
        $("#msg1").text("Full payment is required for card payment");
        return false;
    }
...
$(document).ready(function() {
    ...
    // Check-in and check-out fields are readonly - skip date picker initialization
    // Date values are pre-populated from database settings
});
