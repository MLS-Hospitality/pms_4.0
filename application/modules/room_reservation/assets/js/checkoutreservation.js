...
// CRITICAL FIX: Get base room rent (before discount) for calculations
var roomrent = parseFloat($("#allroomrent").text().replace(/,/g, '')) || 0;
// Get base rent + tax + service charge (before discount) from hidden field
var roomrentandtax = parseFloat($("#oldallroomrentandtax").text().replace(/,/g, '')) || 0;
var allbpccharge = parseFloat($("#allbpccharge").text().replace(/,/g, '')) || 0;
var alladvanceamount = parseFloat($("#alladvanceamount").text().replace(/,/g, '')) || 0;
var poolbill = parseFloat($("#poolbill").text().replace(/,/g, '')) || 0;
var hallbill = parseFloat($("#hallbill").text().replace(/,/g, '')) || 0;
var restbill = parseFloat($("#restbill").text().replace(/,/g, '')) || 0;
var parkingbill = parseFloat($("#parkingbill").text().replace(/,/g, '')) || 0;
...
$("#percent").on("keyup change", function() {
    ...
    if (percent >= 0 && percent <= 100) {
        ...
        // Calculate discount amount on base rent only
        var discountAmount = (roomrent * percent) / 100;
        // Calculate tax and service charge (they are not discounted)
        var taxAndServiceCharge = roomrentandtax - roomrent;
        // Apply discount to base rent only, then add tax and service charge
        var roomrentAfterDiscount = roomrent - discountAmount;
        var totalrentandtax = roomrentAfterDiscount + taxAndServiceCharge;
        // Calculate payable amount correctly
        var payableamount = totalrentandtax + allbpccharge + complementaryamount - alladvanceamount;
        ...
        // CRITICAL FIX: Ensure final payable amount never goes negative
        payableamt = Math.max(0, payableamt);

        $("#amount").val(discountAmount.toFixed(2));
        $("#disamount").text(discountAmount.toFixed(2));
        // Total room rent = base rent - discount
        $("#totalroomrentamount").text(roomrentAfterDiscount.toFixed(2));
        // Total with tax and service charge after discount (base rent - discount + tax + service charge)
...
$("#amount").on("keyup change", function() {
    ...
    var taxAndServiceCharge = roomrentandtax - roomrent;
    // Apply discount to base rent only, then add tax and service charge
    var roomrentAfterDiscount = roomrent - amount;
    var totalrentandtax = roomrentAfterDiscount + taxAndServiceCharge;
    // Calculate payable amount correctly
    var payableamount = totalrentandtax + allbpccharge + complementaryamount - alladvanceamount;
    // CRITICAL FIX: Ensure payable amount never goes negative (fully paid = 0)
    ...
    if (amount >= 0 && amount <= roomrent) {
        ...
        $("#disamount").text(amount.toFixed(2));
        // Total room rent = base rent - discount
        $("#totalroomrentamount").text(roomrentAfterDiscount.toFixed(2));
        // Total with tax and service charge after discount (base rent - discount + tax + service charge)
        $("#allroomrentandtax").text(totalrentandtax.toFixed(2));
...
// CRITICAL FIX: Ensure initial balance never goes negative
payableamt = Math.max(0, payableamt);
$("#balance").text(payableamt.toFixed(2));
"use strict";
$("#additionalcharge").on("keyup change", function() {
    ...
    if (additionalcharge >= 0) {
        var netPayable = payableamount + additionalcharge;
        var finalPayable = payableamount + additionalcharge + poolbill + restbill + hallbill + parkingbill - creditamount - complemetaryamt;
        // CRITICAL FIX: Ensure amounts never go negative
        netPayable = Math.max(0, netPayable);
        finalPayable = Math.max(0, finalPayable);
        $("#netpayableamount").text(netPayable.toFixed(2));
        $("#payableamt").text(finalPayable.toFixed(2));
        $("#balance").text(finalPayable.toFixed(2));
        $("#cash_0").trigger('change');
    }
});
$("#creditamount").on("keyup change", function() {
    ...
    if (creditamount >= 0) {
        var finalPayable = payableamt - creditamount + additionalcharge - complemetaryamt;
        // CRITICAL FIX: Ensure amount never goes negative
        finalPayable = Math.max(0, finalPayable);
        $("#payableamt").text(finalPayable.toFixed(2));
        $("#balance").text(finalPayable.toFixed(2));
