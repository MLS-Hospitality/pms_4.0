...
<div class="row">
    ...
    <img src="<?php echo base_url(!empty($room_image) && !empty($room_image->room_imagename)
        ? $room_image->room_imagename
        : 'assets/img/room-setting/room_images.png'); ?>"
    class="image-inner" alt="">
                        ...
                        <div
                            ...
                            <h2 class="fs-21 mt-3 font-weight-bold">
                                <?php echo display('floor_name').' '.html_escape($floor->floorname ?? 'N/A'); ?>
                                </h2>
                            <h3 class="fs-21 mt-3 font-weight-bold">
                                <?php echo display('room_no')." "; echo html_escape($list->roomno); ?></h3>
                           <p class="mb-1">
    <?php echo display('room_name').' : '.html_escape($room_type->roomtype ?? 'N/A'); ?>
</p>
                           <?php
if ($list->status == 2 || $list->status == 4 || $list->status == 0) {

    $time = $this->db->select("checkoutdate")
        ->from("booked_info")
        ->where("checkindate <=", date("Y-m-d H:i:s"))
        ->where("checkoutdate >=", date("Y-m-d H:i:s"))
        ->where("FIND_IN_SET(".$list->roomno.",room_no)<>", 0)
        ->where_in("bookingstatus", array(0,4))
        ->get()
        ->row();

    if (!empty($time) && !empty($time->checkoutdate)) {

        $diff = strtotime($time->checkoutdate) - strtotime(date("Y-m-d H:i:s"));
        $dur = $time->checkoutdate;
?>
        <p class="mb-1">
            <?php echo display('checkout')." : ".html_escape(date("Y-m-d", strtotime($time->checkoutdate))); ?>
        </p>
