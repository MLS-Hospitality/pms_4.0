    ...
    <div class="row">
        ...
        <div class="col-sm-4">
            <ul class="list-unstyled text-right">
                <?php
                // CRITICAL FIX: Calculate base rent × days first (before discount)
                $baseRent = $totalroom * $total; // Base rent for all days
                $discountAmount = (!empty($disamount->discountamount) ? floatval($disamount->discountamount) : 0); // Discount for all days

                // Calculate tax on BASE RENT (not after discount)
                $totaltax = 0;
                $scharge = 0;
                if(empty($btaxinfo->bookedid)){
                    ...
                    if(!empty($taskname[0])){
                        for($bt=0; $bt<count($taskname); $bt++){
                            $singletax = ($rate[$bt] * $baseRent) / 100;
                            $totaltax += $singletax;
                        }
                    }
                }

                // Calculate service charge on BASE RENT (not after discount)
                ...
                // Calculate subtotal (base rent + tax + service charge)
                $subtotal = ($baseRent + $totaltax + $scharge);

                // Apply discount to get total
                $grprice = $subtotal - $discountAmount;
                ?>
                <li>
                ...
                <?php if(empty($btaxinfo->bookedid)){ ?>
                    <?php if(!empty($taxinfo) && is_array($taxinfo)){ ?>
                        ...
                        <li>
                            <strong><?php echo html_escape($tax->taxname); ?> <?php echo html_escape($tax->rate);?>%:</strong> <?php $singletax=($tax->rate*$baseRent)/100; echo number_format($singletax, 2); ?>
                        </li>
                        <?php } ?>
                    <?php } ?>
                <?php }else{ ?>
                    ...
                    <li>
                        <strong><?php echo html_escape($taskname[$bt]); ?> <?php echo html_escape($rate[$bt]);?>%:</strong> <?php $singletax=($rate[$bt]*$baseRent)/100; echo number_format($singletax, 2); ?>
                    </li>
                    <?php } ?>
                <?php } ?>
                <li>
                    <strong><?php echo display('tax') ?> :</strong> <?php echo number_format($totaltax, 2);?>
                </li>
                <?php if($bookinfo->bookingstatus==5){ ?>
                <?php if(!empty($btaxinfo->scharge) || ($scharge > 0)){ ?>
                <li>
                    <strong><?php echo display("service_charge") ?> :</strong>
                    <?php echo number_format($scharge, 2);?>
                </li>
                <?php } ?>
                <?php } else { ?>
                <?php if($scharge > 0){ ?>
                <li>
                    <strong><?php echo display("service_charge"); ?> :</strong> <?php echo number_format($scharge, 2);?>
                </li>
                <?php } ?>
                <?php } ?>
                <?php if($bookinfo->bookingstatus==5){ ?>
                ...
                <li>
                    <?php
                    // Grand total = (base rent + tax + service charge - discount) + posted bill charges
                    $grand_total = $grprice + $postedbill;
                    ?>
                    <strong><?php echo display('grand_total') ?>:</strong> <?php if($currency->position==1){echo html_escape($currency->curr_icon);}?><?php echo number_format($grand_total, 2);?><?php if($currency->position==2){echo html_escape($currency->curr_icon);}?>
                    <br /><strong><?php echo display('paid_amount') ?>:</strong> <?php if($currency->position==1){echo html_escape($currency->curr_icon);}?><?php if (!empty($bookinfo->paid_amount)){echo number_format($bookinfo->paid_amount+$postedbill-$reducetax, 2);} else { echo "0.00"; }?><?php if($currency->position==2){echo html_escape($currency->curr_icon);}?>
                    <br /><strong><?php echo display('due_amount') ?>:</strong> <?php if($currency->position==1){echo html_escape($currency->curr_icon);}?><?php if (!empty($bookinfo->paid_amount)){$remain_amount = $grand_total - ($bookinfo->paid_amount+$postedbill-$reducetax);if($remain_amount<0){echo "0.00";}else{echo number_format($remain_amount, 2);}} else { echo number_format($grand_total, 2); }?><?php if($currency->position==2){echo html_escape($currency->curr_icon);}?>
                </li>
            </ul>
        </div>
    </div>
