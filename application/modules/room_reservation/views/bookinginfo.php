                ...
                <tbody>
                    ...
                    <tr>
                        <th scope="row"><?php echo display('sub_total') ?><input name="orgdiscount" type="hidden"
                                value="<?php echo html_escape($totalamount);?>" id="orgSubtotal" /></th>
                        <td> <input name="discount" type="hidden" id="discount"
                                value="<?php echo html_escape($totalamount);?>" /><span
                                id="prdis"><?php echo html_escape($totalamount);?></span></td>
                    </tr>
                    <?php 
                    // CRITICAL FIX: Calculate tax and service charge on BASE RENT (not after discount)
                    $tax=0; 
                    $tax=($chargeinfo->vat*$totalamount)/100; 
                    $scharge=0; 
                    $scharge=($chargeinfo->servicecharge*$totalamount)/100; 
                    // Calculate subtotal (base rent + tax + service charge)
                    $subtotal = $totalamount + $tax + $scharge;
                    // Apply discount
                    ...
                    <tr>
                        ...
                        <td><input type="hidden" id="" value="<?php echo html_escape($chargeinfo->vat);?>" /><span
                                id="prtax"><?php echo number_format($tax, 2);?></span>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td><input type="hidden" id=""
                                ...
                                id="prcharge"><?php echo number_format($scharge, 2);?></span>
                        </td>
                    </tr>
                    <?php if($discount > 0){ ?>
                    <tr>
                        <th scope="row"><?php echo display('discount') ?></th>
                        <td><span style="color: #28a745;">-<?php echo number_format($discount, 2);?></span></td>
                    </tr>
                    <?php } ?>
                    <tr>
                        <th scope="row"><?php echo display('grand_total') ?></th><input type="hidden"
                            value="<?php echo html_escape($totalamount);?>" id="orgTotal" /></th>
                        <td><input name="gramount" type="hidden" id="gramount"
                                value="<?php echo html_escape($grandtotal);?>" /><span
                                id="total"><?php echo number_format($grandtotal, 2);?></span></td>
                    </tr>
