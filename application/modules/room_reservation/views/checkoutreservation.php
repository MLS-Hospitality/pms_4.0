        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered table-sm mb-0">
                    ...
                    $allservicecharge = 0; // CRITICAL FIX: Initialize service charge accumulator
                        ...
                        <tr>
                            ...
                            <td>
                                <table class="table table-bordered table-sm mb-0 tr-background">
                                    ...
                                    // Get discount amount from booked_details (per day)
                                    $discountAmountPerDay = !empty($bookingdata[$l]->discountamount) ? floatval($bookingdata[$l]->discountamount) : 0;
                                    ...
                                    ?>
                                        ...
                                        ?></td>
                                            ...
                                            $baseRent = $ratecount[$n] * $days;

                                            // Step 2: Calculate discount on BASE RENT only
                                            $discount = ($ratecount[$n]*$bookingdata[$l]->disrate)/100 * $days;
                                            $offerDiscount = !empty($offer[$n]) ? floatval($offer[$n]) : 0;
                                            $baseRentAfterDiscount = $baseRent - $discount - $offerDiscount;

                                            // Step 3: Calculate tax on BASE RENT (before discount)
                                            $allsingle=0;
                                            foreach($taxsetting as $tax){
                                                $singletax = ($tax->rate * $baseRent) / 100;
                                                $allsingle += $singletax;
                                            }

                                            // Step 4: Calculate service charge on BASE RENT (before discount)
                                            $serviceCharge = ($baseRent * $setting->servicecharge) / 100;

                                            // Step 5: Total = (base rent - discount) + tax + service charge
                                            $total = $baseRentAfterDiscount + $allsingle + $serviceCharge;

                                            // Subtotal for display (base rent + tax + service charge, before discount)
                                            $subtotal = $baseRent + $allsingle + $serviceCharge;
                                            ?>
                                        ...
                                        <?php
                                             // Accumulate base rent
                                             $alltotal += $baseRent;
                                             $alltax += $allsingle;
                                             // Accumulate service charge per room
                                             $allservicecharge += $serviceCharge;
                                             // Accumulate total after discount for display (following correct formula)
                                             $allsubtotal += $total;
                                            ...
                                            // Accumulate base rent (for service charge calculation on base rent)
                                            $allroomrent += $alltotal;
                                            // Accumulate subtotal after discount for display (base rent + tax - discount)
                ...
                <div class="card-body">
                    <table class="table table-sm table-borderless mb-0">
                        <tbody>
                            ...
                            <?php
                            // CRITICAL FIX: Discount applied ONLY to base rent (roomrent), not to subtotal
                            // Calculate total discount applied (from booked_details.discountamount)
                            $totalDiscountAmount = 0;
                            for($discountCalc=0; $discountCalc<count($bookingdata); $discountCalc++){
                                $discDays = ceil((strtotime($bookingdata[$discountCalc]->checkoutdate) - strtotime($bookingdata[$discountCalc]->checkindate)) / (60 * 60 * 24));
                                $totalDiscountAmount += (!empty($bookingdata[$discountCalc]->discountamount) ? floatval($bookingdata[$discountCalc]->discountamount) : 0) * $discDays;
                            }
                            // Apply discount to base rent only
                            $allroomrentAfterDiscount = $allroomrent - $totalDiscountAmount - $promocode;
                            // Total = (base rent - discount) + tax + service charge
                            $allroomrentandtaxFinal = $allroomrentAfterDiscount + $alltax + $allservicecharge;
                            // For display: base rent + tax + service charge (before discount)
                            $allroomrentWithTaxAndService = $allroomrent + $alltax + $allservicecharge;
                            // Use accumulated service charge for display
                            $scharge = $allservicecharge;
                            ?>
                            <tr>
                                <th class="pl-0"><?php echo display("service_charge_amt") ?>.</th>
                                <td><strong><?php if($currency->position==1){ echo html_escape($currency->curr_icon); } ?><span
                                            id="scharge"><?php echo number_format($scharge, 2); ?></span><?php if($currency->position==2){ echo html_escape($currency->curr_icon); } ?></strong>
                                </td>
                            </tr>
                            <tr>
                                <th class="pl-0"><?php echo display("total_room_rent_amt") ?>.</th>
                                <td><strong><?php if($currency->position==1){ echo html_escape($currency->curr_icon); } ?><span
                                            id="totalroomrentamount"><?php echo number_format($allroomrent + $scharge, 2); ?></span><?php if($currency->position==2){ echo html_escape($currency->curr_icon); } ?></strong>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td hidden>
                                    <strong><?php if($currency->position==1){ echo html_escape($currency->curr_icon); } ?><span
                                            id="oldallroomrentandtax"><?php echo number_format($allroomrentWithTaxAndService, 2); ?></span><?php if($currency->position==2){ echo html_escape($currency->curr_icon); } ?></strong>
                                </td>
                                <td><strong><?php if($currency->position==1){ echo html_escape($currency->curr_icon); } ?><span
                                            id="allroomrentandtax"><?php echo number_format($allroomrentandtaxFinal, 2); ?></span><?php if($currency->position==2){ echo html_escape($currency->curr_icon); } ?></strong>
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td><strong>
                                        <?php if($currency->position==1){ echo html_escape($currency->curr_icon); } ?><span
                                            id="payableamount"><?php
                                            // CRITICAL FIX: Ensure payable amount never goes negative (fully paid = 0)
                                            $payableamount = $allroomrentandtaxFinal+$allcomplementarycharge+$allbpccharge-$alladvanceamount;
                                            $payableamount = max(0, $payableamount); // Ensure non-negative
                                            echo number_format($payableamount, 2);
                                            ?></span><?php if($currency->position==2){ echo html_escape($currency->curr_icon); } ?></strong>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                            ...
                            <table class="invp-17">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        // CRITICAL FIX: Discount applied ONLY to base rent (roomrent), not to subtotal
                                        // Step 1: Base rent (no discount)
                                        $inbaseRent = $inrrent[$inr] * $indays;

                                        // Step 2: Calculate discount on BASE RENT only
                                        $intotalDis = ($inrrent[$inr]*$bookingdata[$inl]->disrate)/100;
                                        $indiscount = $intotalDis * $indays;
                                        $inofferDiscount = !empty($offer[$inr]) ? floatval($offer[$inr]) : 0;
                                        $inbaseRentAfterDiscount = $inbaseRent - $indiscount - $inofferDiscount;

                                        // Step 3: Calculate tax on BASE RENT (before discount)
                                        $invat = 0; $singlevat=0;
                                        foreach($taxsetting as $tax){
                                            $invat = ($tax->rate * $inbaseRent) / 100;
                                            $singlevat+=$invat;
                                        }
                                        $allinvat+=$singlevat;

                                        // Step 4: Calculate service charge on BASE RENT (before discount)
                                        $inserviceCharge = ($inbaseRent * $setting->servicecharge) / 100;

                                        // Step 5: Total = (base rent - discount) + tax + service charge
                                        $intotal = $inbaseRentAfterDiscount + $singlevat + $inserviceCharge;

                                        // Subtotal for display (base rent + tax + service charge, before discount)
                                        $insubtotal = $inbaseRent + $singlevat + $inserviceCharge;
                                        ?>
                                        <td class="invp-20">
                                            <?php echo number_format($intotalDis, 2); ?>
                                        </td>
                                        <td class="invp-20">
                                            <?php echo number_format($inrrent[$inr] - $intotalDis, 2); ?>
                                        </td>
                                        <td class="invp-20">
                                        ...
                                        ?>
                                        <td class="invp-20">
                                            <?php echo number_format($invat, 2); ?>
                                        </td>
                                        <?php } ?>
                                        <td class="invp-21">
                                            <?php echo number_format($insubtotal, 2); ?></td>
                                    </tr>
                                </tbody>
                            </table>
                        </td>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="invp-26" width="80">
                                <?php if($currency->position==1){ echo html_escape($currency->curr_icon); } ?><?php echo number_format($allroomrent, 2); ?><?php if($currency->position==2){ echo html_escape($currency->curr_icon); } ?>
                            </td>
                        </tr>
                        <tr>
                            <td class="invp-27">
