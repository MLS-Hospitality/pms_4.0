...
public function customer_pending_bills($customerId = null)
{
	...
	public function posaddtocart(){
				...
				$isgroup1=0;	
					...
					$new_str  = str_replace(',', '0', (string) $addonsid);
					$new_str2 = str_replace(',', '0', (string) $adonsqty);
	...
	public function placeoreder(){
		...
		$this->permission->method('ordermanage','create')->redirect();
		 ...
		 /* add New Order*/
		$purchase_date = str_replace('/', '-', (string) $this->input->post('order_date'));
...
public function ordertoday()
{
    ...
    # Pagination starts
    #
    $config["base_url"] = base_url('ordermanage/order/ordertoday');
    $config["total_rows"]  = $this->order_model->count_order(true); // today only
    $config["per_page"]    = 25;
    ...
    // Make sure orderlist() also filters by today
    $data["iteminfo"] = $this->order_model->orderlist($config["per_page"], $page, true);
